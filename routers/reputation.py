...
import services.reputation as reputation
...
async def api_reputation_current(request: Request):
    ...
    exim = reputation._fetch_exim_window(days=30)
    brevo = reputation._fetch_brevo_complaints(days=30, total_sent=exim['total_sent'])
    ...
    payload = {
        ...
        'data_as_of': time.time(),  # wall-clock when computed; up to 5min stale on cache hit
    }
    _cache['data'] = payload
    _cache['ts'] = now
    return payload
...
async def api_reputation_history(request: Request, days: int = Query(30, ge=1, le=90)):
    _require_auth(request)
    points = reputation.query_history(days=days)
...
async def api_reputation_snapshot(
    ...
    result = reputation.snapshot_now(source='cron')
